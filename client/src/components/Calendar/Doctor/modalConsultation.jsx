...
export const ModalConsulation = ({ show, handleClose }) => {
    ...
    const [type, setType] = useState(consultation?.type || 'virtual');
    ...
    return (
        <Modal show={show} handleClose={handleClose} title={slot._def?.extendedProps.type == 'available' ? 'Agendar consulta' : 'Consulta Programada'}>
                        ...
                        <form className="p-4 md:p-5" onSubmit={handleSubmit}>
                            <div className="grid gap-4 mb-4 grid-cols-2">
                                <div className="col-span-2">
                                    ...
                                    <input type="text" name="patientName" id="name" required onChange={handleChange} value={consultation?.patient ? consultation.patient.firstName + " " + consultation.patient.lastName : null} className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 " />
                                </div>
                                <div className="col-span-2">
                                    <label htmlFor="dni" className="block mb-2 text-sm font-medium text-gray-900 dark:text-white">DNI</label>
                                    <input type="text" name="patientDNI" id="dni" required onChange={handleChange} value={consultation?.patient ? consultation.patient.dni : null} className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 " />
                                </div>
